...
class TankBody extends Live_1.default {
    ...
    tick(tick) {
        ...
        if (this.definition.flags.invisibility) {
            ...
            if (this.inputs.flags & (2 | 8 | 4 | 16) || (this.inputs.movement.x || this.inputs.movement.y) && (!this.collidedWithObject))
        ...
        updateStats: {
            ...
            if (this._currentTank === 51) {
                const statPoints = this.cameraEntity.cameraData.values.statLevels.values[7];
                this.regenPerTick += 1.1 - 0.1 * statPoints;
            }
            if (this.definition.flags.invisibility) {
                const damageReductionFactor = 1.0 - (this.styleData.values.opacity * 0.6);
