    ...
    public onKill(entity: LivingEntity) {
        ...
        if (entity instanceof Square && this.definition.flags.canClaimSquares && this.barrels.length) {
            ...
            if (barrelsToShoot.length) {
                ...
                // Destroy the square once, just like in Diep.io
                entity.destroy(true);
                ...
                // Create two Necromancer Squares (2 drones for 1 square destroyed)
                for (let i = 0; i < 2; i++) {
   ...
   public onDeath(killer: LivingEntity) {
    ...
    public tick(tick: number) {
        ...
        if (this.definition.flags.invisibility) {
            ...
            if (this.inputs.flags & (InputFlags.up | InputFlags.down | InputFlags.left | InputFlags.right) || (this.inputs.movement.x || this.inputs.movement.y)&&(!this.collidedWithObject)) this.styleData.opacity += this.definition.visibilityRateMoving;
        ...
        updateStats: {
            ...
            if (this._currentTank === Tank.Spike) {
                this.damagePerTick += 1; // Only add +1 base damage per tick for Spike
                this.damagePerTick *= 1.5; // Apply the 1.5x multiplier for Spike
            }
            ...
            // Regen (for Spike tank, use the formula +1.1(-0.1*statpoints))
            ...
            if (this._currentTank === Tank.Spike) {
                // Apply the custom health regen formula for Spike tank
                const statPoints = this.cameraEntity.cameraData.values.statLevels.values[Stat.HealthRegen];
                this.regenPerTick += 1.1 - 0.1 * statPoints;  // Adds the health regen adjustment for Spike
            }

            if (this.definition.flags.invisibility) {
                const damageReductionFactor = 1.0 - (this.styleData.values.opacity * 0.6);  // 0% at opacity 0, 60% at opacity 1
        
                this.damagePerTick *= damageReductionFactor;
            }
            // Reload
